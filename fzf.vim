...
fun! s:get_parent_direcotry()
  if s:is_in_git_directory()
    return system('git rev-parse --show-toplevel 2> /dev/null')[:-2]
  else 
    return g:vim_initial_working_dir
  endif
endfun
...
command! -bang -nargs=* PRg
  \ call fzf#vim#grep("rg --column --line-number --no-heading --color=always --smart-case ".shellescape(<q-args>), 1, fzf#vim#with_preview({'dir': s:get_parent_direcotry()}), <bang>0)
